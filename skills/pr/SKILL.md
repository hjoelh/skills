---
name: pr
description: Create a draft GitHub pull request from the current worktree state into the repository's default branch using the gh CLI. Use when the user asks to open or create a PR, wants the base branch to match the repo's main/default branch automatically, wants a short PR summary, or explicitly says to omit a test plan.
---

# PR

Create the draft pull request with `gh`, targeting the repository's default branch unless the user explicitly asks for a different base. Handle detached HEAD worktrees by creating a feature branch before opening the PR.
...
3. Resolve the PR head branch:
   - If `git branch --show-current` returns a branch name, use it.
